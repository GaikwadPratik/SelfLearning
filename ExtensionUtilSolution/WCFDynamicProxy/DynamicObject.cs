...
namespace WCFDynamicProxy
{
    public class DynamicObject
    {
            ...
            object[] parameters)
        {
            if (types.Length != parameters.Length)
                throw new ArgumentException(
                    Constants.ParameterValueMistmatch);

            MethodInfo mi = this.objType.GetMethod(method, types);
            if (mi == null)
                throw new ApplicationException(string.Format(
                    Constants.MethodNotFound, method));
        ...
        {
            return (T)this.CallMethod(method, parameters);
        }

        public async Task<T> CallMethodAsync<T>(string method, params object[] parameters)
        {
            return await (Task<T>)this.CallMethod(method, parameters);
        }
        
        public T CallMethod<T>(string method, Type[] types, object[] parameters)
        {
            return (T)this.CallMethod(method, types, parameters);
        }

        public async Task<T> CallMethodAsync<T>(string method, Type[] types, object[] parameters)
        {
            return await (Task<T>)this.CallMethod(method, types, parameters);
        }
