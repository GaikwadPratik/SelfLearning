...
class DataFrame(object):
    ...
    def from_csv(cls, file_path, delimiting_character=',', quote_character='"'):
        ...
        """
        # opens a file in read, universal newline mode and store the file
        # object in infile for this with block
        with open(file_path, 'rU') as infile:

            # create a csv.reader object to process the file and store it in
            # the variable reader
            ...
            # return an instantiated object of the current class (DataFrame if
    ...
    def __init__(self, list_of_lists, header=True):
        ...
        # if what was passed into header is True or has a value that is
        # equivalent to false, i.e.  bool(header) is True
        if header: # then do this

            # set the header attribute of this DataFrame object that is being
            # instantiated to the first row of what
            # was passed into list_of_lists
            self.header = [_header.strip() for _header in list_of_lists[0]]
            # set the data attribute of this DataFrame object to all the rows
            # after the first row
            # (remember things start from 0 in python not 1)
            self.data = list_of_lists[1:]

        # if what was passed into header is False or has a value that is
        # equivalent to false, i.e.  bool(header) is False
        else: # then do this
            # set the data attr to list_of_lists (there's no header here)
            self.data = list_of_lists

            # create a variable called generated_header and set it as an empty
            # list
            generated_header = []

            # choose the first row of the data set as a sample row to iterate
            # through the columns, enumerate it so
            ...
            for index, column in enumerate(self.data[0]): # for each index and row in this first row of data
                # append a string that is 'column' concatenated with a string
                                                                         # of
                                                                                                                                  # the
                                                                                                                                                                                           # current
                                                                                                                                                                                                                                                    # index
...
#=========================task 2=============================
            tempdata = []           #creating empty array for storing rows
            for row in self.data:
                temprow = []        #creating empty array for storing final striped data as variable in for
                                    #loop is read only
                for cell in row.split(','):
                    temprow.append(cell.strip())
        ...
        for row in self.data:
            # for each iteration of the above loop create an empty list called
            # ordered_dict_data
            ordered_dict_data = []

            # for each index and value of this row in self.data
            for index, row_value in enumerate(row):
                # append a tuple to ordered_dict_data that contains the value
                # in header that's at the same index as
                # this value in row
                ordered_dict_data.append((self.header[index], row_value))

            # outside of the inner loop (for index, row_value in
            # enumerate(row))
            # ordered_dict_data now contains a list of tuples

            # create an OrderedDict using ordered_dict_data and assign it to
            # ordered_dict_row
        ...
        # now ordered_dict_rows has all the data from before but each row is an
        # OrderedDict instead of just a list of
        # values
    ...
    def __getitem__(self, item):
        ...
        # if item is a string or unicode object
            ...
            temp = Series([row[item] for row in self.data]).data
            return temp
    ...
    def checkHead(self, col_name):
       ...
       #     return False
        ...
        if col_name in self.colnames:
            returnVal = self.colnames.index(col_name)
        return returnVal

    def getValuesBuColName(self,col_name):
        headerIndex = self.checkHead(col_name)

        if (headerIndex != -1):
            columnindexName = 'column' + str(headerIndex + 1)            
            nums = self[columnindexName]
            areAllDates = [True if (('/') in val) else False for val in nums]
            allDates = False in areAllDates
            return nums,allDates
        else:
            raise NameError('Column not found')

    def min(self,col_name):
        colValues,isDate = self.getValuesBuColName(col_name)
        if (isDate):
            result = str(min([float(num.replace(',','').strip()) for num in colValues]))
        else:
            result = min([parse(num) for num in colValues]).strftime("%m-%d-%Y %H:%M:%S")
        return str(result)

    def max(self,col_name):
        colValues,isDate = self.getValuesBuColName(col_name)
        if (isDate):
            result = str(max([float(num.replace(',','').strip()) for num in colValues]))
        else:
            result = max([parse(num) for num in colValues]).strftime("%m-%d-%Y %H:%M:%S")
        return str(result)
...
things = datalines[558].split('"')
datalines[558] = things[0] + things[1].replace(',','') + things[-1]


df1 = DataFrame(list_of_lists=data)
df = DataFrame(list_of_lists=datalines, header=False)
df.setHeader(df1.header)
print('Min Price: ' + df.min('Price'))
print('Max Price: ' + df.max('Price'))
print('Min Lat: ' + df.min('Latitude'))
print('Max Lat: ' + df.max('Latitude'))
print('Min Lag: ' + df.min('Longitude'))
print('Max Lag: ' + df.max('Longitude'))
print('Min Tran Date: ' + df.min('Transaction_date'))
print('Max Tran Date: ' + df.max('Transaction_date'))
#print(df.min('Product'))
#print(df.min('Account_Created'))
## get the 5th row
##fifth = df[4]
##sliced = df[4:10]
